...
class Modeling:
    def __init__(self, source, df, file_name, problem_type, type_column, dependent_var):
        self.source = source
    ...
    def time_series(self, encoder, scaler):
        ...
        encoder_path = f'Knowledge/{self.problem_type}/{self.source}/{self.file_name}/models/encoder_{self.dependent_var}.pkl'
        self.dict_manager.update_value(f"Encoder path ", encoder_path)
        joblib.dump(encoder, encoder_path)

        scaler = f'Knowledge/{self.problem_type}/{self.source}/{self.file_name}/models/scaler_{self.dependent_var}.pkl'
        self.dict_manager.update_value(f"Scaler path ", scaler)
...
#         self.X_test = np.reshape(self.X_test, (self.X_test.shape[0], 1, self.X_test.shape[1]))
    ...
    def train_lstm_model(self, epochs=50, batch_size=32, validation_data=None):
        ...
        path = f'Knowledge/{self.problem_type}/{self.source}/{self.file_name}/models/LSTM_model_{self.dependent_var}.h5'
        column_names_path = f'Knowledge/{self.problem_type}/{self.source}/{self.file_name}/models/column_names_{self.dependent_var}.txt'
        
        scaler_x_path = f'Knowledge/{self.problem_type}/{self.source}/{self.file_name}/models/scaler_x_{self.dependent_var}.pkl'
        scaler_y_path = f'Knowledge/{self.problem_type}/{self.source}/{self.file_name}/models/scaler_y_{self.dependent_var}.pkl'
        encoder_path = f'Knowledge/{self.problem_type}/{self.source}/{self.file_name}/models/label_encoder_{self.dependent_var}.pkl'
        
        self.dict_manager.update_value(f"LSTM trained model path", path)
        self.save_model(path)
#         self.dict_manager.update_value(f"Encoder path", encoder_path)
#         joblib.dump(encoder, encoder_path)
#         self.dict_manager.update_value(f"scaler_x_path", scaler_x_path)
#         joblib.dump(scaler, scaler_x_path)
#         self.dict_manager.update_value(f"scaler_y_path", scaler_y_path)
#         joblib.dump(scaler, scaler_y_path)
        ...
        plt.xlabel('Epoch')
        plt.legend()
        path = f'Knowledge/{self.problem_type}/{self.source}/{self.file_name}/graphs/LSTM_training_loss_graph_{self.dependent_var}.png'
...
#         y_test_reconstructed[:,0] = Y_test.ravel()
        ...
        path = f'Knowledge/{self.problem_type}/{self.source}/{self.file_name}/models/evaluation_metrics_{self.dependent_var}.json'
    ...
    def plot_lstm_inference(self):
        ...
        plt.xlabel('Sample Index')
        plt.legend()
        path = f'Knowledge/{self.problem_type}/{self.source}/{self.file_name}/graphs/LSTM_inference_plot_{self.dependent_var}.png'
...
#         print("Random Forest Model Model Accuracy : ", rfc_accuracy)
    ...
    def train_rfc_model(self,X_train, X_test, y_train,  y_test):
        ...
        model_path = f'Knowledge/{self.problem_type}/{self.source}/{self.file_name}/models/RFC_model_{self.dependent_var}.joblib'
        scaler_x_path = f'Knowledge/{self.problem_type}/{self.source}/{self.file_name}/models/scaler_x_{self.dependent_var}.pkl'
        x_encoder_path = f'Knowledge/{self.problem_type}/{self.source}/{self.file_name}/models/x_encoder_{self.dependent_var}.pkl'
        y_encoder_path = f'Knowledge/{self.problem_type}/{self.source}/{self.file_name}/models/y_encoder_{self.dependent_var}.pkl'
        column_names_path = f'Knowledge/{self.problem_type}/{self.source}/{self.file_name}/models/column_names_{self.dependent_var}.txt'
...
#         self.dict_manager.update_value(f"Random Forest Classifier {self.dependent_var[0]} column names path ", column_names_path)
        ...
        accuracy_path = f'Knowledge/{self.problem_type}/{self.source}/{self.file_name}/models/model_accuracy_{self.dependent_var}.txt'
...
#         np.savetxt(accuracy_path, accuracy, fmt='%s', delimiter=', ')
        ...
        cm_csv_path = f'Knowledge/{self.problem_type}/{self.source}/{self.file_name}/csv/confusion_matrix_csv_{self.dependent_var}.csv'
        ...
        cm_path = f'Knowledge/{self.problem_type}/{self.source}/{self.file_name}/graphs/confusion_matrix_{self.dependent_var}.png'
