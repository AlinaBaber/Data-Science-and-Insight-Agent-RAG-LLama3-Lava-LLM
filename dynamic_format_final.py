...
import json
from report_gen import AnalysisGenerator
...
class DataScienceReport:
    def __init__(self, auth_token, df, question,json_file, problem_type, source, file_name):
        if torch.cuda.is_available():
            torch.cuda.init()
...
#         self.generate_word_document()
    ...
    def generate_word_document(self):
        try:
            ...
            output_file_path = f'Knowledge/{self.problem_type}/{self.source}/{self.file_name}/report/{self.file_name}.json'
            ...
            doc = Document()
            json_path = f'Knowledge/{self.problem_type}/{self.source}/{self.file_name}/report/{self.file_name}.json'
            ...
            doc_path = f'Knowledge/{self.problem_type}/{self.source}/{self.file_name}/report/{self.file_name}.docx'
            doc.save(doc_path)
