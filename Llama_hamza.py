...
#         formatted_response2 = response.replace('. ', '.\n').replace('  ', ' ')
    ...
    def intoduction(self, df):
        ...
        SYSTEM_PROMPT = B_SYS + DEFAULT_SYSTEM_PROMPT + E_SYS
        if len(df) <= 10:
            prompt = f"""Generate a Overall general introduction based on the provided dataset {df}.If you encounter uncertainty or lack information, avoid providing speculative details.\n"""
        else:
            prompt = f"""Generate a Overall general introduction based on the provided dataset {df[:30]}.If you encounter uncertainty or lack information, avoid providing speculative details.\n"""
